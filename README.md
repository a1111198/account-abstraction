...
2. A minimal zkSync "Smart Wallet" using native AA
   ...
   2. We _do_ send our zkSync transaction to the alt-mempool
...
```bash
git clone https://github.com/a1111198/account-abstraction-eth-zksync
cd account-abstraction-eth-zksync
make
```
...
> [!IMPORTANT]  
> _Never_ have a private key associated with real funds in plaintext.
...
1. Calls `createAccount` or `create2Account` on the `CONTRACT_DEPLOYER` system contract
   1. This will deploy the contract _to the L1_.
...
The transaction will revert. The `ContractDeployer` checks to see if it knows the hash, and if not, it will revert! The `ContractDeployer` calls the `KnownCodesStorage` contract, which keeps track of _every single contract hash deployed on the zkSync chain. Crazy right!_
